...
fun CompassDial(
    ...
    // Track an unwrapped (non-0..360-clamped) target so the needle always takes the shortest
    // path — without this, crossing the 0°/360° boundary would spin the needle almost a full
    // turn instead of a few degrees, which is very noticeable with live sensor input.
    var continuousTarget by remember { mutableFloatStateOf(bearingDegrees) }
    androidx.compose.runtime.LaunchedEffect(bearingDegrees) {
        var delta = (bearingDegrees - continuousTarget) % 360f
    ...
    Box(modifier = modifier.size(dialSize), contentAlignment = Alignment.Center) {
        Canvas(modifier = Modifier.size(dialSize)) {
            ...
            // Qibla needle, rotated to bearing (0 = up/north)
            rotateCanvas(degrees = animatedBearing, pivot = center) {
                ...
                drawPath(arrowPath, color = BrassLight)
                drawLine(
        ...
        // Direction labels (kept upright, not rotated with the needle)
        ...
        Box(contentAlignment = Alignment.Center) {
            androidx.compose.foundation.layout.Column(horizontalAlignment = Alignment.CenterHorizontally) {
                Icon(Icons.Filled.Explore, contentDescription = null, tint = AmberMuted, modifier = Modifier.size(16.dp))
                Text(
